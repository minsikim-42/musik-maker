...
| 드럼 | (createVoices drums 분기) `makeNoiseBurst` | 킥=MembraneSynth. 스네어·하이햇은 **타격마다 새 ToneBufferSource 원샷(폴리포닉)** — NoiseSynth는 노이즈 소스가 하나뿐이라 자주 치면 소스 재시작 충돌('Start time...' 에러+파열음)이 나서 바꿈. 스네어는 하이패스 350Hz로 저역(폭발음) 제거 |
| 마스터 | `softShape` `makeMaster` `realtimeMaster` | 헤드룸(-6dB, `MASTER_TRIM`) + 넓은범위 소프트클립. WaveShaper는 입력 ±1 초과를 곡선 끝에 '평평하게 잘라'(=퍼벅) 버리므로 입력을 1/`MASTER_DRIVE`로 축소해 넣고 곡선이 ±DRIVE까지 다루게 함 → 겹쳐도 하드클리핑 없음. 천장 0.9라 정상 믹스는 투명 |
